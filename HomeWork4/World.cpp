...
void World::show(Painter& painter) const {
    ...
    // Костыль, убирающий всю пыль после 10 секунд работы программы 
    // (тк в Application::updateWorld() world.update() больше не вызывается и пыль зависает)
    if((dusts.size() > 0) && Dust::DustsShouldBeCleaned())
    {
        this->dusts.clear();
    }
    // Отрисовка пыли
